...
class ProcessImage(QThread):
    signal_show_frame = Signal(object)
    signal_fruit_count = Signal(dict)
    ...
    def run(self):
        ...
        while True:
            ...
            if valid is not True:
                break
            processed_frame, counts = self.detector.detect(self.frame)
            self.signal_show_frame.emit(processed_frame)
            ...
            self.signal_fruit_count.emit(fruit_counts)
            cv2.waitKey(30)
...
class MainWindow(QWidget):
    ...
    def predict(self):
        self.process_image = ProcessImage(self.fileName)
        self.process_image.signal_show_frame.connect(self.show_output)
        self.process_image.signal_fruit_count.connect(self.update_fruit_count)
        self.process_image.start()
    ...
    def update_fruit_count(self, counts):
        count_text = ""
        for fruit, count in counts.items():
            count_text += f"{fruit}: {count}\n"
        self.ui.lbl_fruit_count.setText(count_text)
